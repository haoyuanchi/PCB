function [ contour_space_diff_var,contour_space_diff_ratio,contour_transdist] = BoundarySpaceCompare( template_img,drawback_img,source_img,source_gary_box,template_gary_box)
...
%计算对应点梯度方向差
contour_space_diff_sum=0;
contour_space_diff_count=0;
if isempty(source_correspondence)
    contour_space_diff_var=0;
    contour_space_diff_ratio=0;
    contour_transdist=0;
    return;
end
...
%     end
    ...
    contour_space_diff(i) = ((template_idx_near(1)-source_idx_near(1))^2+(template_idx_near(2)-source_idx_near(2))^2)^(1/2);
    if isnan(contour_space_diff(i)) || isinf(contour_space_diff(i))
        contour_space_diff(i) = 0;
    end
    if contour_space_diff(i)>0.5
        contour_space_diff_sum = contour_space_diff_sum+contour_space_diff(i);
        contour_space_diff_count = contour_space_diff_count+1;
    end
end
...
if contour_space_diff_count==0
    contour_space_diff_var = 0;
else
    contour_space_diff_var= var(contour_space_diff);
end
contour_space_diff_ratio=contour_space_diff_count/size(source_correspondence,2);
