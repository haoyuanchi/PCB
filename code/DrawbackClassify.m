...
%  [ contour_space_diff_var,contour_space_diff_avr,contour_transdist] = BoundarySpaceCompare( template_img,drawback_img,source_img,source_gary_box,template_gary_box);
%  [ drawback_flag ] = BoundaryShapeCompare( template_img,drawback_img,source_img);
[ drawback_flag ] = BoundaryHausdorffDist( template_img,drawback_img,source_img);
...
% space_change_ranpe = ((contour_space_diff_var>3 || contour_transdist >=5) && contour_space_diff_avr>=2.5);
% if  space_change_ranpe && abs(size(template_corner,1)-size(source_corner,1))>=0 && (drawback_gray_average <=0.5)
if  (drawback_flag==1) && (drawback_gray_average <=0.5)
    drawback_class = '毛刺';   
%     subplot(2,2,1);imshow(source_img);title('输入图像局部');
%     subplot(2,2,2);imshow(template_img);title('模板图像局部');
%     subplot(2,2,3);imshow(drawback_pixel);title('不同区域');
...
elseif  (drawback_flag==1) && (drawback_gray_average >=0.5)
    drawback_class = '缺口';
%     subplot(2,2,1);imshow(source_img);title('输入图像局部');
%     subplot(2,2,2);imshow(template_img);title('模板图像局部');
%     subplot(2,2,3);imshow(drawback_pixel);title('不同区域');
